...
class MinimalRNNCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.W = nn.Linear(input_size, hidden_size).cuda()

        self.weight_uh = nn.Parameter(torch.Tensor(hidden_size, hidden_size).cuda())
        self.weight_uz = nn.Parameter(torch.Tensor(hidden_size, hidden_size).cuda())

        self.bias_hh = nn.Parameter(torch.Tensor(hidden_size).cuda())
    ...
    def forward(self, input, hx):
        z = torch.tanh(self.W.cuda()(input))
        u = torch.addmm(self.bias_hh, hx.cuda(), self.weight_uh).cuda()
        u = torch.addmm(u, z, self.weight_uz.cuda())
        u = torch.sigmoid(u)
        return u * hx.cuda()+ (1 - u) * z
