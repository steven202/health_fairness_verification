...
def main(args):
    ...
    save_path = (
        f"run7/model_{args.model}_split_{args.dataset_split}_log_{mode}"
    )
    ...
    for i in tqdm(flatten_combinations([rSubset(list(range(0, 6)), tmp) for tmp in range(1, 7)])):
        ...
        for n_split in range(splits_num):
            ...
            append_metrics("verify", metrics, true_label, predicted)

            metrics["verify"]["MN"].append(mcnemar(true_label, clean_predicted, predicted))
            # FPR, FNR, ACC = calculate_metrics(predicted, true_label)
            # assert not np.isnan(ACC)
