...
# This file is copied from the following repository:
# https://github.com/ThomasYeoLab/CBIG/tree/424e2d9633521b2521d46e4bf22efa74d894f29f/stable_projects/predict_phenotypes/Nguyen2020_RNNAD
# The original file is located at:
# https://github.com/ThomasYeoLab/CBIG/tree/424e2d9633521b2521d46e4bf22efa74d894f29f/stable_projects/predict_phenotypes/Nguyen2020_RNNAD/cbig/Nguyen2020/misc.py
...
def build_pred_frame(prediction, outpath=""):
    ...
    table["Forecast Month"] = np.concatenate([np.arange(len(x)) + 1 for x in dates])
...
def month_between(end, start):
    ...
    to_next = relativedelta(end + relativedelta(months=1, days=-diff.days), end).days
    to_prev = diff.days
...
def gender_convert(value):
    ...
    if value not in book:
        raise Exception(f"cannot convert {value}")
    return book[value]


def latino_convert(value):
    book = {
        "Not Hisp/Latino": 0,
        "Hisp/Latino": 1,
        "Unknown": 2,
...
def DX_conv(value):
    ...
    if isinstance(value, str):
        ...
        if value.endswith("MCI"):
            return 1.0
        if value.endswith("NL") or value.endswith("CN"):
            return 0.0
        if value.endswith("SMC"):
            return 1.0
    return float("NaN")
...
def censor_d1_table(_table):
    ...
    _table.drop(8376, inplace=True)  # Duplicate row for subject 1088 at 72 months
    _table.drop(8586, inplace=True)  # Duplicate row for subject 1195 at 48 months
    _table.loc[12215, "Month_bl"] = 48.0  # Wrong EXAMDATE and Month_bl for subject 4960
...
def load_table(csv, columns):
    ...
    table = pd.read_csv(csv, converters=NEW_CONVERTERS, usecols=columns)
...
NEW_CONVERTERS = {
    ...
    "TAU_UPENNBIOMK9_04_19_17": PET_conv,
    "ABETA_UPENNBIOMK9_04_19_17": PET_conv,
    "PTGENDER": gender_convert,
    "PTETHCAT": latino_convert,
    "PTRACCAT": race_convert,
    "PTMARRY": marriage_convert,
    "DX_bl": DX_conv,
}
